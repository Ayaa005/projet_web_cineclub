...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $isOrg) {
    ...
    if (empty($errors)) {
        ...
        $sm->execute([$movie_id, $org_id]);
        $mv = $sm->fetch();
        if ($mv) {
            $pdo->prepare("
                INSERT INTO sessions(organizer_id,movie_title,movie_poster,session_date,session_time,location,status)
                VALUES(?,?,?,?,?,?,'upcoming')
...
<head>
    ...
    .movie-picker-drop {
        ...
        border-radius: 0 0 4px 4px; max-height: 280px;
    ...
    .mp-preview {
        ...
        padding: 10px 12px; background: rgba(229,9,20,.1);
        border: 1px solid rgba(229,9,20,.3); border-radius: 4px;
    }
...
<div class="container">
    <div class="page-header">
        ...
        <?php if ($isOrg): ?>
        <button class="btn-red" onclick="document.getElementById('m-plan').style.display='flex'">
            + Add Session
        </button>
        <?php endif; ?>
    </div>
        ...
        <div>
            ...
            <div class="sess-list">
                ...
                <div class="card sess-card <?= $i===0 ? 'next-s' : '' ?>">
                    <span class="badge <?= $i===0 ? 'sess-badge-next' : 'sess-badge-planned' ?>">
                        <?= $i===0 ? 'NEXT' : 'PLANNED' ?>
                    </span>
                    <div class="sess-title">
                        <?= htmlspecialchars(strtoupper($s['movie_title'])) ?>
                    </div>
                    <div class="sess-meta">
                        <span>
    ...
    <div class="modal" style="max-width:520px">
        ...
        <form method="POST">
            ...
            <div class="form-field">
                ...
                <div class="mp-preview" id="mp-prev">
                    <img id="mp-prev-img" src="" alt="">
                    <span id="mp-prev-t"></span>
                    <button type="button" onclick="clearMp()">✕</button>
                </div>
            </div>
            ...
            <div class="modal-btns">
                <button type="submit" class="btn-red" id="mp-submit"
